...
class RDPMITM:
    ...
    def __init__(self, mainLogger: SessionLogger, crawlerLogger: SessionLogger, config: MITMConfig, state: RDPMITMState = None, recorder: Recorder = None):
        ...
        # Marvi => add config for sessionID 
        self.tcp = TCPMITM(self.client.tcp, self.server.tcp, self.player.tcp, self.getLog("tcp"), self.state, self.recorder, self.statCounter, self.config)
        """TCP MITM component"""
        ...
        if config.recordReplays:
            ...
            # Marvi
            replayFileName = f"{self.config.sessionId}.pyrdp"
            # Marvi
